...
class VolunteerSkill(SQLModel, table=True):
    ...
    # Relationships
    skill_assignments: List["VolunteerSkillAssignment"] = Relationship(
...
class Volunteer(SQLModel, table=True):
    ...
    total_hours_contributed: Decimal = Field(
    ...
    training_records: List["VolunteerTrainingRecord"] = Relationship(
        back_populates="volunteer"
    )
...
class VolunteerTraining(SQLModel, table=True):
    ...
    duration_hours: Optional[Decimal] = Field(
        default=None, max_digits=4, decimal_places=2, ge=0
    )
...
class VolunteerTrainingRecord(SQLModel, table=True):
    ...
    score: Optional[Decimal] = Field(
        default=None, max_digits=5, decimal_places=2, ge=0, le=100
    )
