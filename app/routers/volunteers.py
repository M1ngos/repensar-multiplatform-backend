...
from app.crud.volunteer import (
    volunteer_crud,
    volunteer_skill_crud,
    volunteer_time_log_crud,
    volunteer_stats_crud,
)
...
def register_volunteer(
    registration_data: VolunteerRegistration, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        for skill_id in registration_data.skill_ids:
            ...
            volunteer_skill_crud.assign_skill_to_volunteer(
                db, volunteer.id, skill_assignment
            )
...
def get_my_volunteer_profile(
    ...
    """Get the volunteer profile for the currently authenticated user."""
    try:
        ...
        profile_data = volunteer_crud.get_volunteer_profile_with_details(
            db, volunteer.id
        )
        if not profile_data:
            raise HTTPException(
...
def get_volunteers(
    ...
    try:
        ...
        for volunteer in volunteers:
            # Get user info
            user_info = volunteer_crud.get_volunteer_profile_with_details(
                db, volunteer.id
            )
            if user_info:
                user = user_info["user"]
...
def update_volunteer_profile(
    ...
    try:
        ...
        updated_volunteer = volunteer_crud.update_volunteer(
            db, volunteer_id, update_data
        )
...
@router.put(
    "/{volunteer_id}/skills/{skill_id}", response_model=VolunteerSkillAssignment
)
...
def get_volunteer_hours(
    ...
    try:
        ...
        if (
            ...
            not in ["admin", "staff_member", "project_manager"]
            and volunteer.user_id != current_user.id
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
def update_volunteer_hours(
    ...
    try:
        ...
        if not time_log:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Time log not found"
            )
        ...
        updated_log = volunteer_time_log_crud.update_time_log(
            db, time_log_id, update_data
        )
        if not updated_log:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
def get_volunteer_hours_summary(
    ...
    try:
        ...
        summary = volunteer_time_log_crud.get_volunteer_hours_summary(
            db, volunteer_id, year
        )
        return summary
...
def get_volunteer_projects(
    ...
    try:
        ...
        return PaginatedResponse[ProjectSummary](
            data=project_summaries, metadata=metadata
        )
...
def get_volunteer_tasks(
    ...
    try:
        ...
        for task in tasks:
            ...
            volunteers_count_query = select(
                func.count(TaskVolunteer.volunteer_id)
            ).where(TaskVolunteer.task_id == task.id)
...
@router.patch("/{volunteer_id}/onboarding", response_model=dict)
def update_volunteer_onboarding(
    volunteer_id: int,
    onboarding_data: VolunteerOnboardingUpdate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Update volunteer onboarding data (volunteer can only update their own)."""
    try:
        volunteer = volunteer_crud.get_volunteer(db, volunteer_id)
        if not volunteer:
            raise HTTPException(
        ...
        # Permission: volunteer updates own profile, admin can update any
        if (
            current_user.user_type.name not in ["admin", "staff_member"]
            and volunteer.user_id != current_user.id
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Not authorized to update this volunteer's onboarding",
            )

        update_fields = onboarding_data.model_dump(exclude_unset=True)
...
def get_volunteer_activity(
    ...
    try:
        ...
        count_query = select(func.count(ActivityLog.id)).where(
            ActivityLog.volunteer_id == volunteer_id
        )
        if action_filter:
