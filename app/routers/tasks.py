...
def _calculate_match_score(
    ...
    volunteer_skill_names: set,
) -> float:
    """
    Return a 0.0–1.0 score representing skill overlap between a task and a volunteer.

    - Returns 1.0 when no specific skills are required (every volunteer qualifies).
    - Returns the proportion of required skill keys that the volunteer possesses
      (case-insensitive key comparison) when skills are required.
    - The caller is responsible for passing an empty set when no volunteer is known,
      which will yield 0.0 for tasks that do require skills.
    """
    if not required_skills:
        # No skill requirements — perfect fit for everyone.
        return 1.0

    required = {str(k).lower() for k in required_skills}
    if not required:
        return 1.0

    matched = required & volunteer_skill_names
    ...
    try:
        ...
        for task in tasks:
            ...
            if task.end_date:
                ...
                is_overdue = days_remaining < 0 and task.status in [
                    "not_started",
                    "in_progress",
                ]
        ...
        logger.error("Failed to retrieve tasks: %s", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def get_task_stats(project_id: Optional[int] = None, db: Session = Depends(get_db)):
    """Get task statistics (public endpoint)."""
    try:
        stats = task_crud.get_task_stats(db, project_id=project_id)
        return TaskStats(**stats)
    except Exception as e:
        logger.error("Failed to retrieve task stats: %s", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve task stats.",
        )
...
def get_volunteer_available_tasks(
    ...
    try:
        tasks_data = task_crud.get_volunteer_suitable_tasks(
            db, volunteer_id, skip, limit
        )

        # --- Skill matching: pre-load volunteer skills once (O(1) DB call) ---
        volunteer_skill_names: set = set()
        if volunteer_id is not None:
            ...
            if volunteer:
                skill_query = (
                    select(VolunteerSkill.name)
                    .join(
                        VolunteerSkillAssignment,
                        VolunteerSkillAssignment.skill_id == VolunteerSkill.id,
                    )
                    .where(VolunteerSkillAssignment.volunteer_id == volunteer_id)
                )
                skill_names = db.exec(skill_query).all()
                volunteer_skill_names = {str(name).lower() for name in skill_names}
        ...
        for data in tasks_data:
            ...
            match_score = _calculate_match_score(
                task.required_skills, volunteer_skill_names
            )
        ...
        logger.error("Failed to retrieve volunteer available tasks: %s", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def get_task(
    ...
    try:
        ...
        for assignment, volunteer, user in task_data["volunteer_assignments"]:
            detail.volunteer_assignments.append(
                TaskVolunteerAssignment(
        ...
        if task.end_date:
            ...
            detail.is_overdue = days_remaining < 0 and task.status in [
                "not_started",
                "in_progress",
            ]
        ...
        logger.error("Failed to retrieve task %s: %s", task_id, e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_task(
    ...
    try:
        ...
        is_manager_or_owner = (
            ...
            or current_user.user_type.name == "staff_member"
        )

        # Volunteers assigned to the task via task_volunteers may update status only
        is_assigned_volunteer = False
        if not is_manager_or_owner and current_user.user_type.name == "volunteer":
            from app.models.volunteer import Volunteer, TaskVolunteer

            self_volunteer = db.exec(
                select(Volunteer).where(Volunteer.user_id == current_user.id)
            ).first()
            if self_volunteer:
                assignment = db.exec(
                    select(TaskVolunteer)
                    .where(TaskVolunteer.task_id == task_id)
                    .where(TaskVolunteer.volunteer_id == self_volunteer.id)
                    .where(TaskVolunteer.is_active == True)
                ).first()
                if assignment:
                    is_assigned_volunteer = True

        if not is_manager_or_owner and not is_assigned_volunteer:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Not authorized to update this task",
            )

        # Volunteers may only change status — strip all other fields
        if is_assigned_volunteer and not is_manager_or_owner:
            allowed = task_data.status
            task_data = TaskUpdate(status=allowed)
        ...
        if task_data.status and task_data.status != old_status:
            ...
            # Add volunteers (get_task_volunteers returns dicts with "volunteer" key)
            for entry in volunteer_assignments:
                vol = entry["volunteer"]
                if vol:
                    users_to_notify.add(vol.user_id)
...
def get_task_volunteers(
    ...
    try:
        ...
        logger.error("Failed to retrieve task volunteers for task %s: %s", task_id, e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve task volunteers.",
        )
...
async def assign_volunteer_to_task(
    ...
    try:
        ...
        is_manager = (
            ...
            or _can_manage_project(current_user, project)
        )
        is_self_signup = False
        if not is_manager and current_user.user_type.name == "volunteer":
            # Volunteers can sign themselves up — verify the volunteer_id belongs to them
            from app.models.volunteer import Volunteer

            self_volunteer = db.exec(
                select(Volunteer).where(Volunteer.user_id == current_user.id)
            ).first()
            if self_volunteer and self_volunteer.id == volunteer_data.volunteer_id:
                is_self_signup = True

        if not is_manager and not is_self_signup:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Not authorized to assign volunteers to this task",
            )

        assignment = task_volunteer_crud.assign_volunteer(
            db, task_id, volunteer_data.volunteer_id
        )
        if not assignment:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="No available volunteer spots for this task.",
            )
        ...
        # Raised by assign_volunteer CRUD for an already-active assignment
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Volunteer is already assigned to this task.",
        )
    except IntegrityError:
        # Safety net: catches any DB unique-constraint violation that slips through
        # (e.g. concurrent requests that both passed the pre-check race).
        # We roll back and return a clean 409 — never leaking raw SQL details.
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Volunteer is already assigned to this task.",
        ...
        logger.error(
            "Unexpected error assigning volunteer %s to task %s: %s",
            volunteer_data.volunteer_id,
            task_id,
            e,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred while assigning the volunteer.",
        )


@router.put(
    "/{task_id}/volunteers/{volunteer_id}", response_model=TaskVolunteerAssignment
)
def update_volunteer_assignment(
    ...
    try:
        ...
        logger.error(
            "Failed to update volunteer assignment (task=%s, volunteer=%s): %s",
            task_id,
            volunteer_id,
            e,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to update volunteer assignment.",
        )
...
def remove_volunteer_from_task(
    ...
    try:
        ...
        logger.error(
            "Failed to remove volunteer %s from task %s: %s", volunteer_id, task_id, e
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to remove volunteer.",
        )
...
def get_task_dependencies(
    ...
    try:
        ...
        logger.error("Failed to retrieve dependencies for task %s: %s", task_id, e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def create_task_dependency(
    ...
    try:
        ...
        if (
            dependency_data.predecessor_task_id != task_id
            and dependency_data.successor_task_id != task_id
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        logger.error("Failed to create dependency for task %s: %s", task_id, e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def delete_task_dependency(
    ...
    try:
        # Fetch the dependency record by its own primary key
        dependency = task_dependency_crud.get_dependency(db, dependency_id)
        if not dependency:
            raise HTTPException(
        ...
        logger.error("Failed to delete dependency %s: %s", dependency_id, e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get(
    ...
    response_model=List[VolunteerTaskAssignment],
)
def get_my_task_assignments(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        if not volunteer:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Volunteer profile not found for current user",
            )

        tasks_data = task_volunteer_crud.get_volunteer_tasks(db, volunteer.id)
        ...
        logger.error("Failed to retrieve task assignments for current volunteer: %s", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve volunteer assignments.",
        )


@router.get(
    "/volunteers/{volunteer_id}/assignments",
    response_model=List[VolunteerTaskAssignment],
)
def get_volunteer_task_assignments(
    ...
    try:
        ...
        logger.error(
            "Failed to retrieve task assignments for volunteer %s: %s", volunteer_id, e
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve volunteer assignments.",
