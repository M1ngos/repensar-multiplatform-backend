    ...
    if project_id:
        ...
        return {
            ...
            "environmental_metrics": [
                m.model_dump() for m, _ in project_data["environmental_metrics"]
            ],
        }
    else:
...
def export_projects_csv(
    ...
    try:
        ...
        projects = project_crud.get_projects(
            db, skip=0, limit=10000, status=status, category=category
        )
        ...
        logger.error("Failed to export projects to CSV: %s", e)
        raise HTTPException(
...
def export_volunteers_csv(
    ...
    try:
        ...
        for volunteer in volunteers:
            ...
            writer.writerow(
                [
                    ...
                    volunteer.date_of_birth.isoformat()
                    ...
                    float(volunteer.total_hours_contributed)
        ...
        logger.error("Failed to export volunteers to CSV: %s", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def export_tasks_csv(
    ...
    try:
        ...
        logger.error("Failed to export tasks to CSV: %s", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        if project_id:
            ...
            export_data = {
                ...
                "milestones": [
                    m.model_dump(mode="json") for m in project_data["milestones"]
                ],
                "environmental_metrics": [
                    {
        ...
        logger.error("Failed to export projects to JSON: %s", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to export projects to JSON.",
        )
